...
var oakSlider = {
    ...
    settings: {
        ...
        autoPlay: true,
        autoPlaySpeed: 4,
        // Increase to stay on slides for longer
        transitionSpeed: 1.75,
        // Changes transition speed
    ...
    init: function() {
        ...
        this.settings.imageScale = gsap.to('.slide--active .slide__image img', 10, {
            scale: 1.1,
            ease: "sine.inOut",
            yoyo: true,
            yoyoEase: true,
    ...
    slideAnimation: function(from, to, dir) {
        ...
        this.settings.timeline.set(to, {
            ...
            stagger: 0.1
        }, 'slideAnimation').fromTo(to + ' .slide__content *', this.settings.transitionSpeed / 100 * 40, {
            y: 20,
            opacity: 0
        }, {
            y: 0,
            ...
            ease: 'Power3.out',
            stagger: 0.1
        }, '>0.5').fromTo(from + ' .slide__image', this.settings.transitionSpeed, {
            display: 'block',
            x: '0%'
        }, {
            x: dir === 'next' ? '-100%' : '100%',
            ease: this.settings.ease + '.inOut'
        }, 'slideAnimation').fromTo(from + ' .slide__image img', this.settings.transitionSpeed, {
            transformOrigin: 'center',
            x: '0%'
        }, {
            x: dir === 'next' ? '50%' : '-50%',
            ease: this.settings.ease + '.inOut'
        }, 'slideAnimation').fromTo(to + ' .slide__image img', this.settings.transitionSpeed, {
            transformOrigin: 'center',
            x: dir === 'next' ? '-50%' : '50%'
        }, {
            x: '0%',
            ease: this.settings.ease + '.inOut'
        }, 'slideAnimation').fromTo(to + ' .slide__image', this.settings.transitionSpeed, {
            display: 'block',
